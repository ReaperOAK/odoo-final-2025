...
const APITester = () => {
  ...
  const runAllTests = async () => {
    ...
    const tests = [
      ...
      // Listings API Tests
      () => testAPI(() => listingsAPI.getListings({ limit: 5 }), 'Listings', 'GET /listings'),
      
      // Orders API Tests
      () => testAPI(() => ordersAPI.getMyOrders({ limit: 5 }), 'Orders', 'GET /orders/my'),
      
      // Host API Tests
      () => testAPI(() => hostsAPI.getHostDashboard(), 'Hosts', 'GET /host/dashboard'),
      () => testAPI(() => hostsAPI.getHostEarnings(), 'Hosts', 'GET /host/earnings'),
      
      // Payouts API Tests
      () => testAPI(() => payoutsAPI.getHostPayouts({ limit: 5 }), 'Payouts', 'GET /payouts'),
      () => testAPI(() => payoutsAPI.getAvailableBalance(), 'Payouts', 'GET /payouts/balance'),
      
      // Admin API Tests (might fail due to permissions)
      () => testAPI(() => adminAPI.getPlatformOverview(), 'Admin', 'GET /admin/platform-overview'),
      () => testAPI(() => adminAPI.getAllUsers({ limit: 5 }), 'Admin', 'GET /admin/users'),
      
      // Host API Tests
      () => testAPI(() => hostsAPI.getHostDashboard(), 'Host', 'GET /host/dashboard'),
      
      // Payments API Tests (might fail if no payment ID, that's expected)
      () => testAPI(() => paymentsAPI.getPaymentHistory({ limit: 5 }), 'Payments', 'GET /payments/history'),
  ...
  const testSpecificEndpoint = async (category) => {
    ...
    switch (category) {
      case 'auth':
        ...
        break;
      case 'listings':
        await testAPI(() => listingsAPI.getListings({ limit: 10 }), 'Listings', 'GET /listings');
        break;
      case 'orders':
        await testAPI(() => ordersAPI.getMyOrders(), 'Orders', 'GET /orders/my');
        break;
      case 'host':
        await testAPI(() => hostsAPI.getHostDashboard(), 'Host', 'GET /host/dashboard');
        break;
      case 'admin':
        await testAPI(() => adminAPI.getAllUsers({ limit: 10 }), 'Admin', 'GET /admin/users');
        break;
      default:
        break;
    }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">API Integration Tester</h1>
