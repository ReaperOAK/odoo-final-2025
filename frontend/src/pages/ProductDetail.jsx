...
import { ArrowLeftIcon, StarIcon, CheckIcon, CubeIcon, ClockIcon } from '@heroicons/react/24/outline'
...
export default function ProductDetail() {
  ...
  const images = product.images && product.images.length > 0 ? product.images : ['https://via.placeholder.com/600x400?text=No+Image']
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center bg-white p-8 rounded-xl shadow-lg max-w-md">
          ...
          <p className="text-gray-600 mb-4">
            Your rental request has been submitted successfully. You'll be redirected to your bookings page.
          </p>
          ...
          <div className="lg:col-span-2">
                    ...
                    {images.map((image, index) => (
                      <button
                        ...
                        className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-colors ${
                          selectedImage === index ? 'border-brand' : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <img
                          src={image}
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 mt-6">
              <div className="mb-6">
                ...
                <div className="mb-6">
                  <span className="text-4xl font-bold text-brand">${basePrice}</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {product.pricing.map((price, index) => (
                      <div key={index} className="border rounded-lg p-4 text-center">
                        <div className="text-2xl font-bold text-brand">${price.rate}</div>
                        <div className="text-sm text-gray-600 capitalize">per {price.unit}</div>
                      </div>
                    ))}
                  </div>
                </div>
