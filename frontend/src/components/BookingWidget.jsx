...
import { CurrencyDollarIcon, ClockIcon, ShieldCheckIcon } from '@heroicons/react/24/outline'
...
export default function BookingWidget({ product, onBook }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 sticky top-4">
      <div className="mb-6">
        <h3 className="text-xl font-bold text-gray-900 mb-2">Reserve this item</h3>
      ...
      {user ? (
        ...
        >
          {loading ? 'Checking...' : canBook ? `Book for $${price?.total}` : 'Select dates to book'}
        </button>
      ) : (
        <div className="space-y-3">
          <p className="text-sm text-gray-600 text-center">Please log in to make a booking</p>
          <a
            href="/login"
